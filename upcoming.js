    ...
    Object.entries(snapshots).forEach(([key, snapshot]) => {
        ...
        snapshot.items.forEach(item => {
            ...
            if (!isRealEnglishMatch(item, series))
                return;
            // Book-number cross-check: a pre-order whose number doesn't match the
            // series' already-computed "next" number is far more likely to be a
            // mismatched/foreign listing than a genuine next release.
            if (typeof series.nextAudioBook === 'number' &&
                typeof item.bookNumber === 'number' &&
                item.bookNumber !== series.nextAudioBook) {
                return;
            }
